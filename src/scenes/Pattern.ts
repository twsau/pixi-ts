...
export class PatternScene extends Container implements IScene {
  ...
  constructor() {
    ...
    this.points = Array.from(Array(100)).map(() => new Graphics());
    this.points.forEach((point) => this.addChild(point));
  }
  ...
  update(_delta: number) {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      point.clear();
      point
        .beginFill(0x00ffff, 0.5 + Math.abs(Math.cos(i * this.state.count / 5000)) * 0.5)
        .drawCircle(
